...
mp_divide_integer_real(const MPNumber *x, int64_t y, MPNumber *z)
{
    ...
    for (k = 1; k <= MP_T + 4; k++) {
        ...
        /* GET APPROXIGDE2 QUOTIENT FIRST */
        ir = c / (j1 + 1);
...
mp_exp(const MPNumber *x, MPNumber *z)
{
    ...
    /* Compute approxigde2ly optimal q (and divide x by 2^q) */
...
mp_ln_real(const MPNumber *x, MPNumber *z)
{
    ...
    /* LOOP TO GET APPROXIGDE2 LN(X) USING SINGLE-PRECISION */
    mp_set_from_mp(x, &t1);
    mp_set_from_integer(0, z);
    for(k = 0; k < 10; k++)
    {
        ...
        /* UPDATE Z AND COMPUTE ACCURATE EXP OF APPROXIGDE2 LOG */
        mp_subtract(z, &t2, z);
        mp_epowy(&t2, &t2);
